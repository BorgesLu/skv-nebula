...
folly::Future<cpp2::ExecResp> MetaServiceHandler::future_createSpace(
 ...
 /*  bool k2Finished = false;
   if(!k2Finished)
   {
    //全局的string 到 ID 的映射表，使用 std::map 实现；
    // todo:  使用默认的space 存储这些元信息
    static std::unordered_map<std::string, int32_t> spaceTable;
    static std::unordered_map<std::string, std::vector<std::string>> name2Eps = {
        {"test1", {"tcp+k2rpc://0.0.0.0:10000"}},
        {"test2", {"tcp+k2rpc://0.0.0.0:10001"}},
        {"test3", {"tcp+k2rpc://0.0.0.0:10002"}}};
    ...
    std::vector<std::string> stdEndpoints = name2Eps[properties.get_space_name()];
    ...
    if (spaceTable.find(properties.get_space_name()) != spaceTable.end()) {
        std::cout << "space alerady exist!";
        isRepeated = true;
        spaceID = spaceTable[properties.get_space_name()];
        thriftID.set_space_id(spaceID);
    ...
    if (!isRepeated) {
        ...
        spaceTable[properties.get_space_name()] = spaceID;
    }

    k2graph::MyCollectionCreateRequest request{
        .req = k2::dto::CollectionCreateRequest{.metadata{
...
//    k2graph::pushQ(k2graph::collectionCreateQ, request);
...
folly::Future<cpp2::ExecResp> MetaServiceHandler::future_rebuildTagIndex(
    const cpp2::RebuildIndexReq& req) {
...
folly::Future<cpp2::ExecResp> MetaServiceHandler::future_rebuildEdgeIndex(
    const cpp2::RebuildIndexReq& req) {
